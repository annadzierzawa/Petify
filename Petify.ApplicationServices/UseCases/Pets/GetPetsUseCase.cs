...
{
    public class GetPetsUseCase : IQueryHandler<GetPetsParameter, List<PetItemDTO>>
    {
        private readonly IPetsQuery _petsQuery;

        public GetPetsUseCase(IPetsQuery petsQuery)
        {
            _petsQuery = petsQuery;
        }

        public async Task<List<PetItemDTO>> Handle(GetPetsParameter query)
        {
            return await _petsQuery.GetPets(query.UserId);
        }
    }
}
